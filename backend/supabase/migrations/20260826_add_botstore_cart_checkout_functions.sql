-- تغییر تعداد سبد با Function اتمیک انجام می‌شود تا کلیک‌های سریع Race Condition ایجاد نکنند.
...
-- Checkout به‌صورت یک تراکنش DB انجام می‌شود: Order + Snapshot Itemها + پاک‌کردن Cart.
...
begin
  ...
  return query
  select v_order_id, v_order_code, v_total, v_item_count;
end;
$$;

-- RPCها از PostgREST قابل مشاهده‌اند ولی Client حق Execute ندارد؛ فقط service_role Backend مجاز است.
